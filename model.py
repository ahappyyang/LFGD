...
class AGCN(nn.Module):
    ...
    def forward(self, X, A):
        H = torch.mm(torch.mm(A, self.w1), A.T)    # 图结构自注意  Ps
        H = torch.mm(torch.mm(H, self.w2), X)  # Pc
        embed = torch.mm(H, H.T)  # S=Pc*Pc.T
        embed = F.normalize(embed, dim=1) # MLP(S)
...
class SCDGN(nn.Module):
    ...
    def forward(self, knn, adj, norm_factor):
        # 联合学习结构和特征
        h = self.AttenGCN(knn,adj)#knn W_knn  adj W  -->H
